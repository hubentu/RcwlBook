...
A `InputParam` constructor is used to define a list of input objects for the command tool, such as "prefix" for the parameter flags and "position" for the paramter orders. More descriptions are given in the CWL specification <https://www.commonwl.org/v1.0/CommandLineTool.html#CommandLineBinding>.

Here is an example from the CWL user guide(<http://www.commonwl.org/user_guide/03-input/>). Here we defined an `echo` with different type of input parameters by `InputParam`. The `stdout` option can be used to caputre the standard output stream to a file.
...
The `stdout` option usually is a string or an expression of output
file name from the command tool. The command's standard output stream
will be captured to a file written to the designated output
...
the output, the name of which is passed from the input parameter "p2". 
